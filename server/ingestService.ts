...
export async function insertEstate(estate: DeceasedEstate): Promise<void> {
  await query(`INSERT INTO estates(id,source_id,deceased_name,id_number_masked,date_of_death,gazette_date,province,district,master_office,estate_number,executor_name,executor_contact,executor_email,value_band,asset_types,raw_notice_snippet,gazette_ref,status,has_property,property_details,date_of_birth,last_address,spouse_details,executor_address,claim_period_days,gazette_number,gazette_page,source_url,parser_version)
    VALUES(${Array.from({length:29},(_,i)=>`$${i+1}`).join(',')}) ON CONFLICT(source_id) DO NOTHING`, [estate.id,estate.sourceId,estate.deceasedName,estate.idNumberMasked,estate.dateOfDeath,estate.gazetteDate,estate.province,estate.district,estate.masterOffice,estate.estateNumber,estate.executorName,estate.executorContact,estate.executorEmail,estate.valueBand,estate.assetTypes,estate.rawNoticeSnippet,estate.gazetteRef,estate.status,estate.hasProperty,estate.propertyDetails||null,estate.dateOfBirth||null,estate.lastAddress||null,estate.spouseDetails||null,estate.executorAddress||null,estate.claimPeriodDays||null,estate.gazetteNumber||null,estate.gazettePage||null,estate.sourceUrl||null,estate.parserVersion||null]);
}
