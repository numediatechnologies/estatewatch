...
export const AdminDataQualityView: React.FC = () => {
  ...
  const load = () => { setLoading(true); void fetchAdminDataQuality().then(setReport).finally(() => setLoading(false)); };
  useEffect(load, []);

  if (loading && !report) return <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 text-xs text-slate-400">Loading Gazette data quality…</div>;
  if (!report) return <div className="rounded-2xl border border-rose-500/30 bg-rose-950/20 p-5 text-xs text-rose-300">Data quality report could not be loaded.</div>;
  ...
  return <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 space-y-4">
    <div className="flex items-center justify-between gap-3">
      <div><div className="flex items-center gap-2 text-amber-400 font-bold text-sm"><Database className="w-5 h-5" />Gazette data quality</div><p className="mt-1 text-[11px] text-slate-400">Live records are limited to the latest four months by Gazette publication date. Legacy rows remain restricted to quarantine.</p></div>
      <button type="button" onClick={load} className="rounded-xl border border-slate-700 p-2 text-slate-300 hover:text-white" aria-label="Refresh data quality"><RefreshCw className="w-4 h-4" /></button>
    </div>
