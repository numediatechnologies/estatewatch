...
export const EstateWatchFooter: React.FC = () => {
  ...
  return <footer className="mt-10 border-t border-slate-800 bg-slate-950" aria-label="EstateWatch eye footer">
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-10 space-y-8">
      <section id="contact" className="grid grid-cols-1 lg:grid-cols-[1.35fr_0.85fr] gap-6">
        <form onSubmit={submit} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
          <div><h2 className="text-lg font-bold text-white">Contact <BrandName /></h2><p className="text-xs text-slate-400 mt-1">Need help with alerts, account access or estate notices? Send us your request and we’ll help with the next step.</p></div>
      ...
      <div className="border-t border-slate-800 pt-6 grid grid-cols-1 md:grid-cols-[1.3fr_1fr_1fr] gap-6 text-xs">
        <div><div className="flex items-center gap-2 text-white font-bold"><span className="w-8 h-8 rounded-lg bg-amber-500/15 text-amber-400 flex items-center justify-center"><Eye className="w-4 h-4" /></span><BrandName /></div><p className="text-slate-500 mt-3 leading-relaxed">Clear estate notices. Practical alerts. A simple next step.</p></div>
        <div><h3 className="text-slate-300 font-bold mb-3"><BrandName /></h3><div className="space-y-2 text-slate-500"><a className="block hover:text-amber-300" href="#contact">Contact us</a><a className="block hover:text-amber-300" href="#popia">POPIA & legal basis</a><a className="block hover:text-amber-300" href="mailto:sales@marketdirect.co.za">Sales support</a></div></div>
        <div><h3 className="text-slate-300 font-bold mb-3">Legal</h3><p className="text-slate-500 leading-relaxed"><BrandName /> is operated by NuMedia Direct Marketing (Pty) Ltd.</p><div className="flex gap-3 mt-3"><a className="text-slate-500 hover:text-amber-300" href="#popia">Privacy</a><a className="text-slate-500 hover:text-amber-300" href="#popia">Terms</a></div></div>
      </div>
      <div className="border-t border-slate-900 pt-4 text-[11px] text-slate-600 flex flex-col sm:flex-row justify-between gap-2"><span>© {new Date().getFullYear()} NuMedia Direct Marketing (Pty) Ltd. All rights reserved.</span><span><BrandName /> · by MarketDirect.co.za</span></div>
    </div>
  </footer>;
};
